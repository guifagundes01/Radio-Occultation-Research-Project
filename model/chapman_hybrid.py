...
class ProfileCorrector(nn.Module):
    ...
    def __init__(self, profile_size, feature_size, hidden_sizes=[128, 64]):
        ...
        cnn_output_size = 128  # After global average pooling
        
        # Combined input size (CNN output + features + Chapman parameters)
        combined_size = cnn_output_size + feature_size + 7  # 7 Chapman parameters
    ...
    def forward(self, profile, features, chapman_params):
        # Process profile through CNN
        profile = profile.unsqueeze(1)  # Add channel dimension
        profile_features = self.profile_encoder(profile)
        profile_features = profile_features.squeeze(-1)  # Remove last dimension
        
        # Combine all features
        combined = torch.cat([profile_features, features, chapman_params], dim=1)
        
        # Generate corrections
        corrections = self.dense(combined)
        
        # Scale corrections based on target profile magnitude
        profile_scale = torch.mean(torch.abs(profile), dim=1, keepdim=True)
        max_correction_scale = 0.01  # Maximum correction is 1% of profile magnitude
...
def train_hybrid_model(model_stage1, model_stage2, train_loader, val_loader, 
                      criterion, optimizer_stage1, optimizer_stage2, 
                      num_epochs=30, device='cuda'):
    """Train both stages of the hybrid model"""
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch, profiles_batch, altitude_batch in train_loader:
            ...
            try:
                # Stage 1: Predict Chapman parameters
                optimizer_stage1.zero_grad()
                chapman_params = model_stage1(X_batch)
                
                # Scale the parameters to prevent overflow
                stage1_loss = criterion(chapman_params, y_batch) * stage1_scale
                if torch.isnan(stage1_loss) or torch.isinf(stage1_loss):
                    continue
                ...
                if any(torch.isnan(p.grad).any() or torch.isinf(p.grad).any() 
                      for p in model_stage1.parameters() if p.grad is not None):
                    continue
                
                # Clip gradients for stage 1
                torch.nn.utils.clip_grad_norm_(model_stage1.parameters(), max_grad_norm)
                optimizer_stage1.step()
                
                # Stage 2: Generate corrections
                optimizer_stage2.zero_grad()
                with torch.no_grad():
                    chapman_profiles = generate_chapman_profiles(chapman_params.detach(), altitude_batch)
                    if torch.isnan(chapman_profiles).any() or torch.isinf(chapman_profiles).any():
                        continue
                
                corrections = model_stage2(profiles_batch, X_batch, chapman_params.detach())
                if torch.isnan(corrections).any() or torch.isinf(corrections).any():
                    continue
                
                # Apply corrections with safety checks
                corrected_profiles = chapman_profiles + corrections
                
                # Scale the loss for Stage 2 using robust MSE
                stage2_loss = robust_mse_loss(corrected_profiles, profiles_batch) * stage2_scale
                
                if torch.isnan(stage2_loss) or torch.isinf(stage2_loss):
                    continue
                ...
                # Check for NaN/Inf gradients
                if any(torch.isnan(p.grad).any() or torch.isinf(p.grad).any() 
                      for p in model_stage2.parameters() if p.grad is not None):
                    continue
                
                # Clip gradients for stage 2
                torch.nn.utils.clip_grad_norm_(model_stage2.parameters(), max_grad_norm)
                optimizer_stage2.step()
                
                # Update loss statistics (divide by scales to get original values)
                total_loss = (stage1_loss.detach() / stage1_scale + 
                            stage2_loss.detach() / stage2_scale)
                train_loss += total_loss.item()
                stage1_loss_sum += stage1_loss.item() / stage1_scale
                stage2_loss_sum += stage2_loss.item() / stage2_scale
                batch_count += 1
        ...
        with torch.no_grad():
            for X_batch, y_batch, profiles_batch, altitude_batch in val_loader:
                try:
                    ...
                    # Stage 1
                    chapman_params = model_stage1(X_batch)
                    stage1_loss = criterion(chapman_params, y_batch)
                    
                    # Stage 2
                    chapman_profiles = generate_chapman_profiles(chapman_params, altitude_batch)
                    corrections = model_stage2(profiles_batch, X_batch, chapman_params)
                    corrected_profiles = chapman_profiles + corrections
                    stage2_loss = robust_mse_loss(corrected_profiles, profiles_batch)
        ...
        if val_batch_count > 0 and val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'optimizer_stage1_state_dict': optimizer_stage1.state_dict(),
                'optimizer_stage2_state_dict': optimizer_stage2.state_dict(),
                'train_loss': train_loss,
                'val_loss': val_loss,
            }, f'{model_dir}/best_hybrid_model.pth')
...
def main():
    ...
    optimizer_stage1 = torch.optim.Adam(model_stage1.parameters(), lr=0.001)
    optimizer_stage2 = torch.optim.Adam(model_stage2.parameters(), lr=0.001)
    
    # Train the model
    train_losses, val_losses, stage1_losses, stage2_losses = train_hybrid_model(
    ...
    print("Training completed! Model and scalers have been saved in the hybrid_model directory.")
