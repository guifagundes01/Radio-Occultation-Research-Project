...
def create_frame(utc_datetime, model_stage1, model_stage2, X_scaler, y_scaler, 
    ...
    if param_type == 'nmf2':
        ...
        title = f'Predicted NmF2 (V3) - UTC: {utc_datetime.strftime("%Y-%m-%d %H:%M")}'
    else:  # hmf2
        values = hmf2
        cmap = 'plasma'
        vmin, vmax = 250, 450
        label = 'hmF2 [km]'
        title = f'Predicted hmF2 (V3) - UTC: {utc_datetime.strftime("%Y-%m-%d %H:%M")}'
...
def create_worldmap_animation(param_type='nmf2', hours_step=1, output_path=None):
    ...
    """
    print(f"Creating V3 animation for {param_type} with {hours_step}-hour steps...")
    ...
    try:
        ...
        if output_path is None:
            output_path = f'{MODEL_DIR}/animation_{param_type}_v3_24h.gif'
...
def create_both_animations(hours_step=1):
    ...
    print("Creating NmF2 animation (V3)...")
    nmf2_path = create_worldmap_animation('nmf2', hours_step)
    
    print("Creating hmF2 animation (V3)...")
    hmf2_path = create_worldmap_animation('hmf2', hours_step)
    
    return nmf2_path, hmf2_path

# Function to create comparison animations between V2 and V3
def create_comparison_animations(hours_step=1):
    """Create side-by-side comparison animations of V2 vs V3."""
    print("Creating comparison animations between V2 and V3 models...")
    
    # Import V2 models for comparison
    ...
    # Load both models
    X_scaler_v3, y_scaler_v3 = load_scalers()
    model_stage1_v3, model_stage2_v3 = load_model(altitude)
    ...
    try:
        ...
        for i, utc_datetime in enumerate(time_sequence):
            ...
            nmf2_v2, hmf2_v2 = generate_predictions_for_time(
                utc_datetime, model_stage1_v2, model_stage2_v2, X_scaler_v2, y_scaler_v2,
            ...
            # V2 plot
            values_v2 = np.log10(nmf2_v2)
            sc1 = ax1.scatter(
                lon_grid.flatten(), lat_grid.flatten(),
                c=values_v2, cmap='jet', s=40, edgecolor='k', 
                transform=ccrs.PlateCarree(), vmin=5, vmax=6.5
            )
            ax1.coastlines()
            ax1.set_global()
            ax1.set_title(f'V2 Model - UTC: {utc_datetime.strftime("%Y-%m-%d %H:%M")}')
            ...
            ax2.coastlines()
            ax2.set_global()
            ax2.set_title(f'V3 Model - UTC: {utc_datetime.strftime("%Y-%m-%d %H:%M")}')
            
            # Add colorbar
            cb = plt.colorbar(sc2, ax=[ax1, ax2], orientation='horizontal', pad=0.05, aspect=50)
            cb.set_label('log10 NmF2 [el/cm³]')
            
            plt.suptitle(f'NmF2 Comparison: V2 vs V3 - {utc_datetime.strftime("%Y-%m-%d %H:%M")}')
            plt.tight_layout()
        ...
        output_path = f'{MODEL_DIR}/animation_comparison_v2_v3_24h.gif'
...
if __name__ == '__main__':
    # Create V3 animations with 2-hour steps (12 frames total)
    print("Creating V3 model animations...")
    nmf2_path, hmf2_path = create_both_animations(hours_step=2)
    print(f"\nV3 Animations created:")
