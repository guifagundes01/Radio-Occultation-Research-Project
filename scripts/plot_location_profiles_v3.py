...
import h5py
import os
from model.chapman_hybrid_v3 import ChapmanPredictor, ProfileCorrector, generate_chapman_profiles
import datetime
...
def load_model_and_scalers():
    """Load the hybrid V3 model and scalers"""
...
def generate_diurnal_predictions(model_stage1, model_stage2, X_scaler, y_scaler, altitude, 
                               latitude, longitude, timezone_offset=0, base_date='2023-06-19'):
...
def plot_location_profiles(profiles, altitude, hours, location_name, save_dir='./data/fit_results/location_analysis_v3'):
    ...
    plt.title(f'{location_name} - Electron Density Profiles Throughout the Day (V3 Model)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{location_name.lower().replace(" ", "_")}_profiles_diurnal_combined_v3.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Electron Density (cm⁻³)', fontsize=12)
    plt.ylabel('Altitude (km)', fontsize=12)
    plt.title(f'{location_name} - Representative Electron Density Profiles (V3 Model)', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=10)
    
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{location_name.lower().replace(" ", "_")}_profiles_representative_v3.png', dpi=300, bbox_inches='tight')
    plt.show()

def plot_diurnal_variation(nmf2_values, hmf2_values, hours, location_name, save_dir='./data/fit_results/location_analysis_v3'):
    ...
    ax1.set_title(f'Diurnal Variation of NmF2 - {location_name} (V3 Model)')
    ...
    ax2.set_title(f'Diurnal Variation of hmF2 - {location_name} (V3 Model)')
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{location_name.lower().replace(" ", "_")}_diurnal_variation_v3.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print summary statistics
    print(f"\n=== {location_name} Diurnal Variation Summary (V3 Model) ===")
...
def compare_v2_v3_predictions(latitude, longitude, location_name, timezone_offset=0, base_date='2023-06-19'):
    """Compare predictions between V2 and V3 models for the same location"""
    print(f"Loading both V2 and V3 models for comparison...")
    
    # Import V2 models
    from model.chapman_hybrid_v2 import ChapmanPredictor as ChapmanPredictorV2, ProfileCorrector as ProfileCorrectorV2
    from model.chapman_function import generate_chapman_profiles
    
    MODEL_DIR_V2 = './data/fit_results/hybrid_model'
    
    # Load V2 model and scalers
    with h5py.File(DATA_PATH, 'r') as f:
        altitude = f['altitude'][:]
    
    X_scaler_v2 = np.load(f'{MODEL_DIR_V2}/X_scaler.npy', allow_pickle=True).item()
    y_scaler_v2 = np.load(f'{MODEL_DIR_V2}/y_scaler.npy', allow_pickle=True).item()
    ...
    model_stage1_v3, model_stage2_v3, X_scaler_v3, y_scaler_v3, _ = load_model_and_scalers()
    
    # Generate predictions for both models
    print(f"Generating predictions for both models...")
    nmf2_v2, hmf2_v2, profiles_v2, local_hours = generate_diurnal_predictions(
        model_stage1_v2, model_stage2_v2, X_scaler_v2, y_scaler_v2, altitude,
        latitude, longitude, timezone_offset, base_date
    )
    
    nmf2_v3, hmf2_v3, profiles_v3, _ = generate_diurnal_predictions(
        model_stage1_v3, model_stage2_v3, X_scaler_v3, y_scaler_v3, altitude,
        latitude, longitude, timezone_offset, base_date
    )
    
    # Create comparison plots
    save_dir = './data/fit_results/location_analysis_comparison'
    os.makedirs(save_dir, exist_ok=True)
    
    # Compare NmF2 predictions
    plt.figure(figsize=(12, 8))
    plt.plot(local_hours, nmf2_v2, 'b-o', linewidth=2, markersize=6, label='V2 Model')
    plt.plot(local_hours, nmf2_v3, 'r-s', linewidth=2, markersize=6, label='V3 Model')
    plt.xlabel('Local Time (hours)')
    plt.ylabel('NmF2 (cm⁻³)')
    plt.title(f'NmF2 Comparison: V2 vs V3 - {location_name}')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.xticks(np.arange(0, 25, 2))
    
    # Add statistics
    max_v2 = np.max(nmf2_v2)
    max_v3 = np.max(nmf2_v3)
    min_v2 = np.min(nmf2_v2)
    min_v3 = np.min(nmf2_v3)
    
    plt.text(0.02, 0.98, f'V2 Max: {max_v2:.1e}\nV3 Max: {max_v3:.1e}\nRatio: {max_v3/max_v2:.2f}', 
    ...
    plt.figure(figsize=(12, 8))
    plt.plot(local_hours, hmf2_v2, 'b-o', linewidth=2, markersize=6, label='V2 Model')
    plt.plot(local_hours, hmf2_v3, 'r-s', linewidth=2, markersize=6, label='V3 Model')
    plt.xlabel('Local Time (hours)')
    plt.ylabel('hmF2 (km)')
    plt.title(f'hmF2 Comparison: V2 vs V3 - {location_name}')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.xticks(np.arange(0, 25, 2))
    
    # Add statistics
    max_hmf2_v2 = np.max(hmf2_v2)
    max_hmf2_v3 = np.max(hmf2_v3)
    min_hmf2_v2 = np.min(hmf2_v2)
    min_hmf2_v3 = np.min(hmf2_v3)
    
    plt.text(0.02, 0.98, f'V2 Range: {max_hmf2_v2-min_hmf2_v2:.0f} km\nV3 Range: {max_hmf2_v3-min_hmf2_v3:.0f} km\nDiff: {max_hmf2_v3-max_hmf2_v2:.0f} km', 
            transform=plt.gca().transAxes, verticalalignment='top', 
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    ...
    noon_idx = 12
    profile_v2 = profiles_v2[noon_idx]
    profile_v3 = profiles_v3[noon_idx]
    
    plt.semilogx(profile_v2, altitude, 'b-', linewidth=2.5, label=f'V2 Model (12:00 LT)')
    plt.semilogx(profile_v3, altitude, 'r-', linewidth=2.5, label=f'V3 Model (12:00 LT)')
    
    plt.xlabel('Electron Density (cm⁻³)', fontsize=12)
    plt.ylabel('Altitude (km)', fontsize=12)
    plt.title(f'{location_name} - Noon Profile Comparison: V2 vs V3', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    # Add statistics
    nmf2_v2_noon = np.max(profile_v2)
    nmf2_v3_noon = np.max(profile_v3)
    hmf2_v2_noon = altitude[np.argmax(profile_v2)]
    hmf2_v3_noon = altitude[np.argmax(profile_v3)]
    
    plt.text(0.02, 0.98, f'V2: NmF2={nmf2_v2_noon:.1e}, hmF2={hmf2_v2_noon:.0f}km\nV3: NmF2={nmf2_v3_noon:.1e}, hmF2={hmf2_v3_noon:.0f}km\nRatio: {nmf2_v3_noon/nmf2_v2_noon:.2f}', 
            transform=plt.gca().transAxes, verticalalignment='top', 
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    ...
    print(f"\n=== {location_name} V2 vs V3 Comparison Summary ===")
    print(f"NmF2 - V2 Max: {max_v2:.1e} cm⁻³, V3 Max: {max_v3:.1e} cm⁻³, Ratio: {max_v3/max_v2:.2f}")
    print(f"hmF2 - V2 Range: {max_hmf2_v2-min_hmf2_v2:.0f} km, V3 Range: {max_hmf2_v3-min_hmf2_v3:.0f} km")
    print(f"Noon Profile - V2 NmF2: {nmf2_v2_noon:.1e}, V3 NmF2: {nmf2_v3_noon:.1e}, Ratio: {nmf2_v3_noon/nmf2_v2_noon:.2f}")

def analyze_location_diurnal(latitude, longitude, location_name, timezone_offset=0, base_date='2023-06-19'):
    """Main function to run diurnal analysis for any location using V3 model"""
    print(f"Loading V3 model and altitude data for {location_name}...")
...
def main():
    ...
    # Run V3 analysis
    print("=== Running V3 Model Analysis ===")
    ...
    print("\n=== Running V2 vs V3 Comparison ===")
    compare_v2_v3_predictions(
        latitude=SAO_LUIS_LAT,
        longitude=SAO_LUIS_LON,
        location_name="São Luís",
        timezone_offset=SAO_LUIS_TIMEZONE
